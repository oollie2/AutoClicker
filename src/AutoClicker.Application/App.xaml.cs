...
public partial class App : Application
{
    ...
    private async void Application_Startup(object sender, StartupEventArgs e)
    {
        ...
        if (Settings<AutoClickerSettings>.Main.CheckForUpdates)
        {
            Updater updater = new();
    ...
    /// <summary>
    /// Simply run an update in a separate process and close the application.
    /// </summary>
    private static void UpdateApplication(string downloadPath)
    {
        if (File.Exists(downloadPath))
        {
            ...
            {
                FileName = "cmd.exe",
                Arguments = "/C msiexec.exe /l*v \"" + Environment.ExpandEnvironmentVariables(Settings<AutoClickerSettings>.Main.ExceptionLogLocation) +
                "upgrade.log\" /qn /i \"" + downloadPath + "\" && " +
    ...
    SettingsFile = Environment.ExpandEnvironmentVariables(@"%APPDATA%\" +
        ...
        return await Settings<AutoClickerSettings>.Read(SettingsFile);
    }
}
